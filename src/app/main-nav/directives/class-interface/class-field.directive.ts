...
@Directive({
  selector: '[appClassField]'
})
export class ClassFieldDirective {
  ...
  ngOnInit(): void {
    InteractJS(this.element.nativeElement)
      ...
      .resizable({
        ...
        listeners: {
          move(event) {
            ...
            elemArray[2].setAttribute('height', event.rect.height + 'px');
            /*
      ...
      .on('dragmove', (event) => {//dodac to do kazdego elementu svg grupy Class i pozniej rozszerzanie na podstawie wielkosci tekstu SVG
        ...
        let transformRects = (event.currentTarget.parentElement.parentElement.children[0].children[2].style.transform).match(/-[[0-9]{1,}|[[0-9]{1,}/g);
