...
@Directive({
  selector: '[appClassClassname]'
})
export class ClassClassnameDirective {
    ...
    InteractJS(this.element.nativeElement)
      ...
      .resizable({
        ...
        listeners: {
          move(event) {
            ...
            let transformClass = (elemArray[0].style.transform).match(/-[[0-9]{1,}|[[0-9]{1,}/g);
            if (transformClass != null) {
              transformed = true;
            }

            if (transformed) {
              elemArray[0].setAttribute('height', event.rect.height + 'px');
              elemArray[1].setAttribute('y', (event.rect.bottom - offsetTop) - (+transformClass[1]));
              elemArray[2].setAttribute('y', (event.rect.bottom - offsetTop + (parseFloat(elemArray[1].getAttribute('height'))) - (+transformClass[1])));
            }
            else {
              //console.log(parseFloat(elemArray[1].getAttribute('height')));

              elemArray[0].setAttribute('height', event.rect.height + 'px');
              elemArray[1].setAttribute('y', (event.rect.bottom - offsetTop));
              elemArray[2].setAttribute('y', (event.rect.bottom - offsetTop + (parseFloat(elemArray[1].getAttribute('height')))));
            }

            for (let i = 0; i < elemTextArray[2].children.length; i++) {

              elemTextArray[2].children[i].setAttribute('y', +elemArray[1].getAttribute('y') + (15 + (15 * i)));
              elemTextArray[2].children[i].setAttribute('x', +elemArray[1].getAttribute('x') + 4);
              if (transformClass != null) {
                elemTextArray[2].children[i].style.transform = 'translate(' + transformClass[0] + 'px, ' + transformClass[1] + 'px)';
              }
            }
      ...
      .on('dragmove', (event) => {//dodac to do kazdego elementu svg grupy Class i pozniej rozszerzanie na podstawie wielkosci tekstu SVG
        ...
        for (let i = 0; i < elemArray.length; i++) {
          ...
          if ((startX + x + marginPixels + 150) >= target.parentElement.parentElement.parentElement.clientWidth) {//RESIZE RIGHT
          ...
          if ((startY + y + marginPixels + 100) >= target.parentElement.parentElement.parentElement.clientHeight) {//RESIZE BOTTOM
